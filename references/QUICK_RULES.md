...
| H2L_flyStrink | HTF→LTF shrink chain active | Shrink propagating — compression cascade in progress | Scenario S/E (compression) |
...
| L2H_sideway | LTF→HTF sideway chain | LTF being suppressed by HTF — D1 active | Scenario S/E (LTF confined) |
...
| `H2L_sideway:3` + all L2H = -1 | C4/H — BOTTOM | No entry — wait M5 BBUpDn 0→1 |
...
| `L2H_flyUP:3` + `H2L_flyUP:0` | P3/B3 — Full chain restored | → Scenario F, full size |
...
**Conflict state:** Both `HTF_Drive_LTF_Sideway` and `LTF_Drive_HTF_Fly` active simultaneously at a TF = volatile transition (C3 or H). sizeMultiplier = 0.5. Watch M5 BBUpDn_state: 0→1 resolves the conflict in favour of expansion.
...
| 4+ | Four or more TFs squeezed | C4/H — BOTTOM | All gates locked — wait |
...
| -1 | 3+ | No shrink, 3+ TFs squeezed | C4/H — BOTTOM | No entry — wait M5 expand |
...
**Pink zone condition:** cas_sqzCount ≥ 2 AND M15+M30 both SQZ simultaneously → G0b-PINK fires → EXIT all. This maps to C2 (LTF full SQZ) or deeper.
...
| Shrink (513) | Shrink (513) | REVERSAL WARNING — both HTF losing direction | B→E→C4 (HTF reversal) | High |
...
|-----|------|-------------------------------------|------|
| F1 | 1 | W1+D1+H4 all 511/512, H1+M30 511/512, M15+M5 511/512 | → B when M15 enters 513 |
| F2 | 1 | H4 511/512 fly but W1 or D1 counter-trend, H1+M30 511/512, M15+M5 511/512 | → B when M15 enters 513 |
| F3 | 1 | H4+D1+W1 all 511/512, H1+M30 511/512, M15 513/523 briefly or M5 400-499 briefly | → B if M15 513 sustains |
| S1 | 2 | D1+H4+H1+M30 all 511/512, M15 513/523, M5 511/512 | → C if M30 also 513, → P if M5 breaks SQZ same dir |
| S2 | 2 | D1+H4+H1 all 511/512, M30 513/523, M15 513/523, M5 511/512 | → C if H1 also 513, → P if M5 breaks SQZ same dir |
| S3 | 2 | D1+H4 511/512, H1 513/523, M30+M15 513/523, M5 511/512 | → C if H4 also 513, → P if M5 breaks SQZ same dir |
| C1 | 2 | H4 511/512, H1 511/512 or 513, M30 513/523 (BBUpDn=2), M15 400-499, M5 400-499 | → C2 if M30 SQZ, → C3 if M5 BBUpDn 0→1 |
| C2 | 2 | H4 511/512, H1 511/512 or 513, M30 400-499 (BBUpDn=0), M15 400-499, M5 400-499 | → C3 if M5 BBUpDn 0→1, → C4 if H4 400-499 |
| C3 | 2 | H4 511/512, H1 511/512, M30 513/523 (BBUpDn 2→1), M15 513, M5 breaking SQZ | → P/B if M15 mid confirms, → C4 if H4 compresses |
| C4 | 2 | H4 513/523 or 400-499, H1 400-499, M30 400-499, M15 400-499, M5 400-499 | → V (direction pivot) |
| V1 | 2 | D1 511/512, H4 breaking SQZ (BBUpDn 0→1 same dir as D1 mid) | → F (compression release) |
| V2 | 2 | D1 511/512, H4 breaking SQZ (BBUpDn 0→4 opposite D1 mid) | → C (trend reversal) |
| V3 | 2 | H4 BBUpDn 1→0 reverts within 3 bars | → back to C/V |
| V4 | 2 | H4 SQZ, BBUpDn alternating 1 and 4 | → Phase 6 if persists |
| P1 | 3 | H4 511/512, H1 511/512, M30/M15 513/523 or 400-499, M5 breaking SQZ | → D2 if M15 mid confirms |
| P2 | 3 | H4 511/512, H1 511/512, M30 511/512 or 513, M15 FLAT→UP/DN, M5 511/512 | → P3 if M30 511/512 |
| P3 | 3 | H4+H1+M30+M15+M5 all 511/512 | → F (full fly) |
| B1 | 3 | H4 400-499 or 513, H1 513 or 400-499, M30 513 or 400-499, M15 511/512, M5 511/512 | → B2 if M30 511/512, WAIT (quality capped) |
| B2 | 3 | H4 400-499 or 513, H1 511/512, M30 511/512, M15 511/512, M5 511/512 | → B3 if H4 511/512 |
| B3 | 3 | H4+H1+M30+M15+M5 all 511/512 (new direction) | → F (full fly) |
| R1 | 3 | H4 511/512 (original dir), H1 still original or reversing, M30+M15 reversed 521/522 | → R2 if H4 flips new dir, WAIT |
| R2 | 3 | H4 flipped new direction 511/512, all TFs new direction | → new F (full fly) |
| R3 | 3 | H4 reversed but W1/D1 still original direction | → R2 if D1 also reverses |
...
| Collapsed to noise (no legs) | ≈ zero at minimum | SQZ confirmed | Phase 4 | C2/E3→H — wait for direction |
...
| E | H4 enters SQZ (400-499) | C4 → H |
| E | M5 BBUpDn 0→1 | B1 (LTF breakout) |
...
| D | M30 BBUpDn=1 | A (P3 confirmed) |
| D | M15 mid flips back to 3 | B (D stalled) |
| F | H4 BBUpDn=1 sustained | A (B3) |
| F | H4 BBUpDn reverts | E/H (false breakout) |
| C | H4 BBUpDn=1 new direction | new A (R2) |
| C | W1/D1 still original | R3 (counter-trend) |
...
M15 + M30 BOTH SQZ simultaneously → EXIT ALL (S2 pink zone). This is the ONLY condition that forces exit of existing positions.
