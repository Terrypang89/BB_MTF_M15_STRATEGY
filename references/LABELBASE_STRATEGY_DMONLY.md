# Label-Based Strategy P&L Report — dm-only Variant

## Provenance

**dm-only**: entries and exits key ONLY on diffMid_Trend (dm). W_stage is NOT used for entry.  
**Sideways exit**: EA-logged TofySideway S_ flag from [BBTFImpact] lines (1684 bars with S_n sub-type).  
**Reversal exit**: when dm flips opposite to current position (LONG→DN or SHORT→UP).

FLY_PSHRINK, SHRINK, and SQZ exits are REMOVED. The only exits are S_ sideway and opposite-dm reversal.

Based on the TofySideway variant's parser (`scripts/labelbase_strategy_tofysideway.py`).
import re
        ...
        for raw in f:
            ...
            if m:
                ...
                m15_data.append({
                    ...
                    "dbw": float(dbw_cur),  # diffBBW current
                    "dbw_prev": float(dbw_prev1),  # diffBBW previous (for FLY_PSHRINK)
                    "bbupdn": bbupdn_cur,  # BBUpDn cur
                })
