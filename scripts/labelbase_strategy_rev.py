...
def evaluate_bar(m15: Dict, m5_price: Optional[float], position: Optional[str]) -> Tuple[Optional[str], float]:
    ...
    if position != "FLAT":
        ...
        # Exit conditions (priority 5-6) — reversal on opposite fly while in position
        # CLOSE only — no immediate re-entry same bar; next bar may re-enter if flat
        if position == "LONG" and ws in {521, 522} and dm in {2, 4}:
            return "FLAT", EXIT_REASON_NAMES["REVERSAL"]
        if position == "SHORT" and ws in {511, 512} and dm in {1, 5}:
            return "FLAT", EXIT_REASON_NAMES["REVERSAL"]

    # Entry conditions (priority 7-8) — only apply when flat and no entry pending
...
def generate_report(trades: List[Dict]) -> str:
    ...
    forced_close_count = sum(1 for t in trades if t["exit_reason"] == EXIT_REASON_NAMES["REVERSAL"])
