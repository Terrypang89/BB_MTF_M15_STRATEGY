...
def baseline_reversion_rate(d: dict, target: float) -> bool:
    """Baseline: directional reversion — only count if price reaches midline
    in the direction implied by its starting position.
    (close < midline => expect UP; close > midline => expect DOWN)."""
    # Determine expected direction from starting position
    if d["close"] is None or target is None:
        return False
    prev_side = "below" if d["close"] < target else "above"
    for _ in range(HORIZON_BARS):
        if prev_side == "below" and d["close"] >= target:
            return True
        if prev_side == "above" and d["close"] <= target:
            return True
    return False
