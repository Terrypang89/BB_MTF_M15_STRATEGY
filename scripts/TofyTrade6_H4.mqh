...
//| V36.15 — H4-flip entry.                                          |
//|   Enters on H4 band-state flip (C->F=UP, F->R=DOWN) within     |
//|   12 H4 bars, instead of M30-confirmed. Tests whether a band-   |
//|   state flip on a SLOW timeframe predicts direction, where       |
//|   M15/M30-confirmed/H4-filtered flips all failed OOS.           |
//|   H4 is 16x slower than M15 — expect FEW trades (~13 per 4      |
//|   months). n<20 per clean window is the LIKELY outcome, making  |
//|   verdict INCONCLUSIVE by sample size. Build it anyway; report  |
//|   n honestly; do NOT stretch a low-n result into a conclusion.  |
//| V36.13 — EXIT-RETRY FIX: pos.open cleared only on confirmed-flat.|
//|   exit_pending retry loop re-issues act=7 until broker closes.  |
//|                                                                    |
//| PART 4 — H4-FLIP DETECTOR (C->F / F->R triggers)                 |
//| PART 5 — v1 Trade Ruleset (gate, entry, TP, SL, invalidation)   |
//| Prediction fields (pred/predmtf/slope/predhit) = DIAGNOSTIC ONLY|
//|   — NOT trade inputs.                                              |
//| Repo logic file, sibling to TofyTrade6.mqh —                     |
...
//═══════════════════════════════════════════════════════════════════
// SIG LOGGING — same grammar as TofyTrade6 (colon-separated KV)
//═══════════════════════════════════════════════════════════════════
...
//═══════════════════════════════════════════════════════════════════
// PART 4 — TransitionDetector trigger struct (H4-FLIP V36.15)
//═══════════════════════════════════════════════════════════════════
struct TransitionTrigger {
   bool     active;              // true if a trigger fired
   int      direction;           // 1=UP, 2=DOWN
   datetime fire_time;           // M5 bar time when trigger fired (H4 flip first seen)
...
struct PositionState {
   ...
   double   tp_price;            // take-profit price (H4 band)
   double   sl_price;            // stop-loss price (opposite H4 band)
   bool     h4_followed;        // has H4 reached trigger target state?
   int      bars_since_entry;    // M5 bars elapsed since entry
   string   trigger_state;       // H4 state at trigger ("F" for UP, "R" for DOWN)
...
//═══════════════════════════════════════════════════════════════════
// PART 4 — H4-FLIP DETECTOR (C->F / F->R triggers)
// FIRES on H4 state flip to F (UP) or R (DOWN) only.
// S/C flips excluded. No HTF filter.
// Validity = 12 H4 bars from fire.
//═══════════════════════════════════════════════════════════════════
// Static: previous H4 state (for flip detection)
static string s_prevH4State = "";

// V36.15 — Pending-trigger tracking for H4-confirmed entry.
// Tracks the bar index of each H4 flip; enters only when H4 confirms
// within 12 bars and h4_state still == direction (no revert).
static int    s_h4_pending_dir   = 0;       // 1=UP, 2=DOWN, 0=none
static datetime s_h4_flip_time     = 0;      // M5 bar time of last qualifying H4 flip

// V36.15: Check H4 confirmation against the pending H4 flip window.
// Returns: -1 = stale (clear it), 0 = no trigger, 1 = UP confirmed, 2 = DOWN confirmed.
int ConfirmH4(DualTFScenarioState &s)
{
   if(s.htf_h4_state == "X")
      return 0;

   //--- No pending flip → nothing to confirm (unless same-bar below)
   if(!s_h4_pending_dir && !s_h4_flip_time)
      return 0;

   int dir = -1;

   // UP: h4 still F AND H4 now F
   if(s.htf_h4_state == "F" && s.htf_h4_state == "F")
   {
      // If we have a pending flip, check staleness (iBarShift for M5 precision)
      if(s_h4_pending_dir > 0 || s_h4_flip_time > 0)
      {
         int barsSince = iBarShift(_Symbol, PERIOD_M5, s_h4_flip_time);
         if(barsSince >= 12) return -1;  // stale — clear it
      }
      dir = 1;
   }

   // DOWN: h4 still R AND H4 now R
   else if(s.htf_h4_state == "R" && s.htf_h4_state == "R")
   {
      if(s_h4_pending_dir > 0 || s_h4_flip_time > 0)
      {
         int barsSince = iBarShift(_Symbol, PERIOD_M5, s_h4_flip_time);
...
TransitionTrigger DetectH4Flip(DualTFScenarioState &s, datetime curTime)
{
   ...
   if(s_prevH4State == "")
   {
      s_prevH4State = s.htf_h4_state;
      return trig;
   }

   // Both current and previous must be non-X (valid data)
   if(s.htf_h4_state == "X" || s_prevH4State == "X")
   {
      s_prevH4State = s.htf_h4_state;
      return trig;
   }
   ...
   if(s.htf_h4_state == s_prevH4State)
   {
      return trig;
   }

   // Flip to F = UP trigger
   if(s.htf_h4_state == "F" && s_prevH4State != "F")
   {
      trig.active = true;
      trig.direction = 1;  // UP
      ...
      // V36.15: record flip for H4-confirmation window
      s_h4_pending_dir = 1;
      s_h4_flip_time   = curTime;
   }
   // Flip to R = DOWN trigger
   else if(s.htf_h4_state == "R" && s_prevH4State != "R")
   {
      trig.active = true;
...
//═══════════════════════════════════════════════════════════════════
string EvaluateGate(int triggerDir, int h4bbloc, bool positionOpen)
{
   ...
   {
      // H4 zone gate: NEAR/MID for UP — mid-range not at-band
      // UP requires h4bbloc in mid-range (3-7) not at-band (0,1 or 9,10)
      if(h4bbloc >= 3 && h4bbloc <= 7)
         return "PASS";
      if(h4bbloc == 0 || h4bbloc == 1 || h4bbloc == 9 || h4bbloc == 10)
   ...
   {
      // DOWN mirror: mid-range not at-band
      if(h4bbloc >= 3 && h4bbloc <= 7)
         return "PASS";
      if(h4bbloc == 0 || h4bbloc == 1 || h4bbloc == 9 || h4bbloc == 10)
         return "GATE_SKIP_ATBAND";
...
void LogTradeEntry(string dir, string gateResult, double entryPrice, double slPrice,
                   double tpPrice, int h4bbloc, string h4State, string h4Combo,
                   datetime dt)
{
...
void LogTradeExit(string reason, double exitPrice, int barsHeld, bool h4Followed,
                  datetime dt)
{
   string dtStr   = TimeToString(dt, TIME_DATE|TIME_SECONDS);
   ...
   Print("[TRADE] evt:EXIT reason:", reason,
         ...
         " h4_followed:", h4Followed ? "Y" : "N");
}

void LogTradeSkip(string reason, string dir, int h4bbloc, datetime dt)
{
   string dtStr   = TimeToString(dt, TIME_DATE|TIME_SECONDS);
...
//═══════════════════════════════════════════════════════════════════
// TRADE STRATEGY — V36.15: identify + H4-Flip detector + v1 ruleset
...
)
{
   ...
   if(!pos.open)
   {
      ...
      pos.tp_price=0; pos.sl_price=0; pos.h4_followed=false;
   ...
   if(pos.open && BUYS == 0 && SELLS == 0)
   {
      ...
      if(pos.exit_pending)
      {
         ...
         LogTradeExit(pos.exit_reason, px, pos.bars_since_entry, pos.h4_followed, cur);
   ...
   if(pos.open && !pos.exit_pending)
   {
      ...
      //--- Check h4 follow: has H4 reached the trigger target state?
      // UP trigger (dir=1): target = F; DOWN trigger (dir=2): target = R
      if(!pos.h4_followed)
      {
         if(pos.trigger_dir == 1 && s.htf_h4_state == "F")
            pos.h4_followed = true;
         if(pos.trigger_dir == 2 && s.htf_h4_state == "R")
            pos.h4_followed = true;
      }

      //--- Priority order for exits:
      // 1. TP_HIT, 2. H4_REVERT, 3. TIMEOUT, 4. SL_HIT
      ...
      // 2. H4_REVERT: h4_state != trigger target state
      if(s.htf_h4_state != pos.trigger_state)
      {
         pos.exit_pending = true;
         pos.exit_reason = "H4_REVERT";
         pos.exit_pending_bar = pos.bars_since_entry;
         Trade_act = 7;
         Trade_info = "SIG:SIG evt:EXIT reason:H4_REVERT";
         return;
      }

      // 3. TIMEOUT: 12 H4 bars elapsed AND h4 has not followed
      // Once h4 HAS followed, timeout no longer applies
      if(!pos.h4_followed && pos.bars_since_entry >= 12)
      {
         pos.exit_pending = true;
   ...
   //--- V36.13: Block entry while pos.open or pos.exit_pending (one-position-at-a-time).
   ...
   //--- Detect H4 transition (also sets s_h4_pending_dir/flip_time)
   TransitionTrigger trig = DetectH4Flip(s, cur);

   int dir = 0;

   if(trig.active)
   {
      // Same-bar check: h4 flipped AND h4 already confirms this bar.
      // ConfirmH4 will return the direction (no staleness on same bar).
      int confirmedDir = ConfirmH4(s);
      if(confirmedDir > 0)
         dir = confirmedDir;
   }

   // H4-confirmation from previous flip window
   if(!dir && s_h4_pending_dir)
   {
      int confirmedDir = ConfirmH4(s);
   ...
   if(dir)
   {
      ...
      string gateResult = EvaluateGate(dir, s.h4_bbloc, positionOpen);

      if(gateResult == "PASS")
      {
         double entryPx = close_prices[LA];

         // SL = opposite-side H4 band at entry
         double slPx;
         if(dir == 1)              // UP: SL = H4 lower band
            slPx = BB_datas[4].BBLowLV[LA];
         else                      // DOWN: SL = H4 upper band
            slPx = BB_datas[4].BBUppLV[LA];

         // TP = H4 band at entry
         double tpPx;
         if(dir == 1)              // UP: TP = H4 upper band
            tpPx = BB_datas[4].BBUppLV[LA];
         else                      // DOWN: TP = H4 lower band
            tpPx = BB_datas[4].BBLowLV[LA];

         //--- V36.15: compute confirm-lag (bars between H4 flip and H4 confirm)
         int h4flipBarIdx   = iBarShift(_Symbol, PERIOD_M5, s_h4_flip_time);
         int confirmBarIdx   = 0;
         int barsBetween     = MathMax(0, h4flipBarIdx - confirmBarIdx);
         ...
         //--- V36.15: Log confirm-lag on entry
         Print("[TRADE] evt:H4_CONFIRM_LAG dir:", trigDir,
               " h4_flip_bar:", IntegerToString(h4flipBarIdx),
               " h4_confirm_bar:", IntegerToString(confirmBarIdx),
               " bars_between:", IntegerToString(barsBetween));

         // Log entry
         string h4Combo = ComboSegment(s.htf_h4_state, s.h4_bbloc);
         LogTradeEntry(trigDir, gateResult, entryPx, slPx, tpPx,
                       s.h4_bbloc, s.htf_h4_state, h4Combo, cur);
      ...
      {
         //--- Skipped trigger
         LogTradeSkip(gateResult, trigDir, s.h4_bbloc, cur);
         ...
         Trade_info = "SIG:SIG evt:SKIP dir:" + trigDir +
                      " reason:" + gateResult +
                      " h4bbloc:" + IntegerToString(s.h4_bbloc);
         return;
      }
   }
...
//+------------------------------------------------------------------+
// INTEGRATION NOTES:
// 1. This file is a repo logic file — sibling to TofyTrade6.mqh.
//    User integrates into their EA locally; does not #include TofyTrade6.
// 2. BB_datas[] index convention: 0=M5, 1=M15, 2=M30, 3=H1, 4=H4, 5=D1.
// 3. Call Trade_Strategy() — same signature as TofyTrade6.
// 4. V36.15: H4-Flip detector (Part 4) + v1 trade ruleset (Part 5).
//    First trading version for H4-flip entry — Trade_act nonzero, fixed 0.01 lots.
// 5. Prediction fields (pred/predmtf/slope/predhit) = DIAGNOSTIC ONLY.
//    They do NOT appear in gate/entry/exit logic (grep-checkable).
// 6. Trade_sl = PRICE convention (matches TofyTrade6).
// 7. TP managed in logic — no TP parameter in ENUM_Trade_Act.
// 8. Exit priority: TP_HIT > H4_REVERT > TIMEOUT > SL_HIT.
// 9. Timeout (12 H4 bars) disabled once h4_followed = true.
...
// 14. V36.15: H4-confirmed entry gate. Enters only when h4_state confirms
//     the H4 flip direction within 12 bars and h4_state still == that direction.
//     Confirm-lag logged per trade via [TRADE] evt:H4_CONFIRM_LAG.
//+------------------------------------------------------------------+
